...
import { setPage, setPerPage, setSearchValue } from "../../redux/reducers/mainPageReducer";
...
export const SearchBar = () => {
    ...
    const getParamsFromUrl = (url: string): { query: string | null; per_page: number | null; page_param: number | null } => {
    ...
    const handelSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        searchValue && navigate(`?query=${searchValue}&per_page=${perPage}&page=${page}`) 
    }
    ...
    const getRequestFromUrl = (subject: string | null, per_page: number | null, page_param: number | null) => {
        navigate(`?query=${subject}&per_page=${per_page}&page=${page_param}`);
        dispatch(getProjectCards({page: page_param, perPage: per_page, searchValue: subject}))
    }

    useEffect(() => {
        const { query, per_page, page_param } = getParamsFromUrl(search)
        !search ? getRequestFromState() : getRequestFromUrl(query, per_page, page_param)
