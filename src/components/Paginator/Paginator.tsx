...
import { Box, LeftArrow, RightArrow, StyledButton, StyledPageNumber } from "./style";
  ...
  export const Paginator = () => {
    ...
    const [totalPages, setTotalPages] = useState<number>(Math.ceil(cards.total_count / perPage));
    const pageNumbers = Array.from({ length: totalPages }, (_, index) => index + 1);
    const displayedPageNumbers = pageNumbers.slice(page - 1, page + 2);
  
    const handlePagesChange = (newPage: number) => {
        dispatch(setPage(newPage))
        dispatch(getProjectCards({page: newPage, perPage, searchValue: searchValue}))
